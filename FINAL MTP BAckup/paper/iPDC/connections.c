...
void PMU_process_TCP(unsigned char tcp_buffer[],int sockfd,int pmuid) {
	...
	//	printf("Bingo ---- Config CRC --- %d \n",cfg_crc);
		...
		printf("Recieved DR_Cfg Frame!!!!!!!!!\n");          
              ...
              /*Save DR config file with id,soc*/
		...
		strcpy(pmuFilePath1,"..");
		strcat(pmuFilePath1, "/iPDC/received_files/");
      ...
      long soc_value=to_long_int_convertor(soc_temp);	
        /*FIles from each pmu has to be identified*/ 
		/*Files are sent in parts, distinguished by file no*/
	...
	fileno = to_intconvertor(file_no);
		...
		strcpy(pmuFilePath5,"..");
		strcat(pmuFilePath5, "/iPDC/received_files/");
        ...
        long int count=0;
       while(count<100)   /*Combining files*/
	   		{count++;   if(count==100) count=65535;
