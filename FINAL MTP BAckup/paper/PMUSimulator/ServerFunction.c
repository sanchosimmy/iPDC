...
/* ----------------------------------------------------------------------------	*/
/* FUNCTION  *reset_dr_flag(struct PDC_Details *temp_pdc)     	               					*/
/* ----------------------------------------------------------------------------	*/
void *reset_dr_flag(struct PDC_Details *temp_pdc) /*second_1_flag=0 After 1 Second*/
{   sleep(1);
...
void* UDP_PMU()
{
	...
	while(1)	
	{
		...
		else		/* New datagram has been received */
		{
			...
			if(c[0]  == 0x04) 		/* Check if it is a command frame from PDC */ 
			{	
				c[0] = udp_command[14];
				c[1] = udp_command[15];

				if((c[1] & 0x05) == 0x05 && (c[0] & 0x00) == 0x00)		/* Command frame for Configuration Frame from PDC */
				{ 				
				...
				else if((c[1] & 0x02) == 0x02 && (c[0] & 0x00) == 0x00)		/* Command frame for Turn ON transmission request from PDC */
				{ 				
...
/* ----------------------------------------------------------------------------	*/
/* FUNCTION  *send_dr_frame(struct PDC_Details *single_pdc_node_for_thread)  	               					*/
/* ----------------------------------------------------------------------------	*/
...
/* ------------------------------------------------------------------ */
/* FUNCTION  tcp_send_dr_data(struct PDC_Details *single_pdc_node):   */
/* PAck the generated COMTRADE files in .cfg and .dat frames          */
/* and send it to PDC via TCP                                         */
/* ------------------------------------------------------------------ */
void tcp_send_dr_data(struct PDC_Details *single_pdc_node)
{   

/*Sending Config Frame first*/
    FILE *cfg_filea;
...
/*Find the length of the file*/
  ...
  /* Creating .cgf frame*/
    ...
    byte_by_byte_copy(ptr_temp,DRSYNC_cfg,indexa,2); 
    indexa += 2;
  ...
  if (send(new_fd1,ptr_temp,fsizea+16,0) == -1)
 {
	...
	/*Sending Data Frame first*/
    /*Splitting files into diffrent parts and sending via TCP*/
    /*diff64 : MAximum size of each part*/
...
while(1)
	...
	if(diff<=0) break; 
		...
		{
                char *drframe_dat = malloc(diff+18+1);  
                memset(drframe_dat,'\0',diff+18+1);

                memset(fsizea_char,'\0',2);
                int_to_ascii_convertor(diff+18,fsizea_char);
                ...
                byte_by_byte_copy(ptr_temp,DRSYNC_dat,indexa,2); 
                indexa += 2;
                byte_by_byte_copy(ptr_temp,fsizea_char,indexa,2);
                indexa += 2;
                byte_by_byte_copy(ptr_temp,pmuid,indexa,2);
                ...
                indexa =indexa+diff;    
                chk = compute_CRC(ptr_temp,diff+16);
		...
		{ /*Sending all files except last part*/
                char *drframe_dat = malloc(diff64+19);
                memset(drframe_dat,'\0',diff64+18+1);

                memset(fsizea_char,'\0',2);
                int_to_ascii_convertor(diff64+18,fsizea_char); 
                ...
                for(int i1 = 0;i1<diff64; i1++) 
                ptr_temp[indexa+ i1] = string_dat_filea[i1+diff64*(filecount-1)];	


                indexa =indexa+diff64;   
                chk = compute_CRC(ptr_temp,diff64+16);
                drframe_dat[indexa++] = (chk >> 8) & ~(~0<<8);  	/*CHKSUM high byte;*/ 
                drframe_dat[indexa++] = (chk ) & ~(~0<<8);     	/*CHKSUM low byte;*/  
...
{ /*
We are only using TCP to send DR data
*/
return(0);
}
/**************************************** End of File *******************************************************/
