...
/* It opens the raw data file, reads the values, Creates the buffer and        */
...
char data_file_path[200];
        char buff4[50];
		memset(data_file_path, '\0', 200);
		strcat(data_file_path, "../DataDir/data");
		sprintf(buff4, "%d",file_no);
		strcat(data_file_path, buff4);
		strcat(data_file_path, ".csv");
		data_file_path[strlen(data_file_path)] = '\0';

    fp=fopen(data_file_path, "r");  /*Opening Raw data file*/
    printf("\n%s\n",data_file_path);
    if (!fp) 
    {
...
/* Makes .cfg file  and stores it as /share/pmu_pmuid.cfg                                                         */
/* ------------------------------------------------------------------------*/
void make_config()
{ 
    int flag_space=0;
...
/* ------------------------------------------------------------------------*/
/* FUNCTION  write_data()                                                  */
/* Makes .dat file and stores it as /share/pmu_pmuid.dat                                                        */
/* ------------------------------------------------------------------------*/
void write_data()
{
    FILE *dat_file;
    ...
    if(tail != NULL) 
    {
        ...
        {
            sampleno++;     
            write_data_helper(ptr->data,sampleno,dat_file,sampleno);
            ptr = ptr->next;
        }
        sampleno++;
        write_data_helper(ptr->data,sampleno,dat_file,sampleno);
...
/* ------------------------------------------------------------------------*/
/* FUNCTION  write_data_helper())                                          */
/* Helps write_data() to write data                                        */
/* ------------------------------------------------------------------------*/
void write_data_helper(char* temp,long int sampleno,FILE* dat_file,long int spno)
{ 
    int field_count = 0;
    float a;
    int digitalcount=0;
    ...
    while (field) 
    {
        ...
        else if(field_count<=phnmr+1)/*Analog values*/
        {
            a=strtod(field , NULL);
			//  if(spno>48500)
			//  {printf("\n%d : ",field_count);     //Delete
			//  printf("%f",a); 
			//  sleep(1);}
            fwrite(&a,sizeof(char),4,dat_file);
        }
        else if(field_count>phnmr+1)/*Digital values*/
        {
            a=strtod(field , NULL);
