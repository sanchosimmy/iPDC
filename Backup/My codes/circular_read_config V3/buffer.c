...
void display_config();
void write_data();
void write_data_helper(char* data,float i,FILE* fff);
...
if(tail != NULL) 
	...
	while(ptr->next != ptr) 
		{sampleno++;     
         	write_data_helper(ptr->data,sampleno,fff);
         	printf("\n");
         	ptr = ptr->next;
      		}
      	sampleno++;
...
void write_data_helper(char* temp,float sampleno,FILE* fff)
{ 		int field_count = 0;
        	float a;
        	...
        	while (field) {
         			if(field_count == 0)  
     	          			{ printf("%f,", sampleno); 
     	          			  fwrite(&sampleno,sizeof(char),4,fff);
     	          			  }           
     			else 
	    			{//a=string2num(field);
	    			a=strtod(field , NULL);
	    			fwrite(&a,sizeof(char),4,fff);
            			printf("%f,", a);}
